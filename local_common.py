...
def cv2_frame_count_manual(path):
    video = cv2.VideoCapture(path)
    fc = 0 # Number of frames read
    while True:
        (grabbed, frame) = video.read() # Grabs, decodes and returns the next video frame
        if not grabbed:
            break # If cam is disconnected or there are no more frames, methods return false and the functions return NULL pointer
        fc += 1
    video.release() # Closes video file or capturing device
    assert is_int(fc)
    return fc
def cv2_frame_count(path):
    video = cv2.VideoCapture(path)
    fc = None
    ...
    if int(major_ver)  < 3 :
        fc = int(video.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT))
        #print "Frames per second using video.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT): {0}".format(fc)
    else :
        fc = int(video.get(cv2.CAP_PROP_FRAME_COUNT))
        #print "Frames per second using video.get(cv2.CAP_PROP_FRAME_COUNT) : {0}".format(fc)
